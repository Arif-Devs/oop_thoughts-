import { Pool } from "pg";
import { DatabaseConfig, DrizzleClient, IDatabaseClient } from "./IDatabaseClient";
...
import * as schema from '@/db/schemas'
 ...
 export class DatabaseClientPool implements IDatabaseClient{
    ...
    async executeQuery<T>(label: string, queryFn: (db: DrizzleClient) => Promise<T>): Promise<T> {
        ...
        try {
            ...
            const duration = performance.now() - start
            console.log(`[${label}] failed in ${duration.toFixed(2)}ms`);
            console.log(error);
